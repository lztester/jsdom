...
function patchGlobalScope(scope) {
  if (scope && scope.globals) {
    ...
    const { body } = scope, index = body.findIndex(
...
function patchGlobalFunctionDeclaration(globalDeclaration) {
  ...
  // In non-strict mode code, functions can only be declared at top level, inside a block, as the body of an if statement,
  // or inside a labelled statement inside block, at top level, or inside another labelled statement inside them.
  ...
  switch (parent.type) {
    ...
    case "LabeledStatement":
      if (!scope || scope.type !== "Program" && scope.type !== "BlockStatement" && scope.type !== "SwitchStatement") {
        break;
      }
